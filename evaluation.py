...
def build_linear_model(pdb_filename):
    ...
    model_structure = pepdesMD.initialize_res(model_structure_geo[0])
    for i in range(1, len(model_structure_geo)):
        model_structure = pepdesMD.add_residue(
...
def build_phi_psi_model(pdb_filename):
    ...
    model_structure_omega = pepdesMD.make_structure(
        seq, phi_diangle, psi_diangle, omega_diangle
    )
    model_structure_phi_psi = pepdesMD.make_structure(
